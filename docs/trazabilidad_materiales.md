...
| Evolución por década y matriz de correlación | `d3.scalePoint`, `d3.scaleLinear`, `d3.scaleOrdinal(d3.schemeTableau10)`, `d3.extent`, `d3.line` y data join, todos vistos en clase | Desarrollo nuevo: gráficos de apoyo sugeridos por el enunciado («scatterplots, histograms, correlation matrices»). |
...
- Estilo de código: las utilidades de D3 se limitan a las vistas en clase (`scaleLinear`, `scalePoint`, `scaleOrdinal`, `extent`, `sum`, `line`, ejes y data join) y a los cálculos con `map`/`filter`/`sort` del notebook (`euclidean`, `kNearestNeighbors`); `d3.brush` y `d3.drag` se añaden porque el enunciado exige brushing y arrastre.
